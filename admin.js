...
function verificarAccesoAdmin() {
  ...
  if (!datos) { window.location.href = "login.html"; return false; }
  const sesion = JSON.parse(datos);
  if (sesion.rol !== "admin") { window.location.href = "index.html"; return false; }
  estadoAdmin.sesion = sesion;
  return true;
}

// ── CARGA LIGERA: sin archivos_pdf para evitar timeout ───────
...
async function crearSemanasIniciales() {
  ...
  try {
    const { error } = await window.supabaseClient.from('semanas').insert(semanas);
    if (error) throw error;
...
function renderizarInfoAdmin() {
  ...
  if (navAdminNombre && estadoAdmin.sesion) navAdminNombre.textContent = estadoAdmin.sesion.nombre;
  if (navAdminRol) navAdminRol.textContent = "Administrador";
}

function mostrarVista(vista) {
  estadoAdmin.vistaActual = vista;
  ["vistaDashboard", "vistaSemanas", "vistaEditor", "vistaUnidades"].forEach(function (id) {
...
function actualizarSidebar(vista) {
  document.querySelectorAll(".sidebar-item").forEach(function (item) {
    ...
    if (item.dataset.vista === vista) item.classList.add("active");
  });
}
...
function renderizarResumenSemanas() {
  ...
  estadoAdmin.semanas.slice(0, 8).forEach(function (semana) {
    ...
    item.style.cssText = "display:flex;align-items:center;justify-content:space-between;padding:10px 14px;border-radius:8px;background:var(--bg-card);border:1px solid var(--border-color);margin-bottom:6px;font-size:0.86rem;";
    item.innerHTML = `
      ...
      <div style="display:flex;gap:6px;align-items:center;">
        <span class="badge ${semana.publicado ? "badge-success" : "badge-muted"}">${semana.publicado ? "Publicada" : "Borrador"}</span>
...
// ── EDITAR: carga archivos_pdf solo al abrir el editor ───────
async function editarSemana(numero) {
  const semana = estadoAdmin.semanas.find(s => s.numero_semana === numero);
  if (!semana) return;

  // Cargar archivos_pdf y contenido_html solo ahora (datos pesados)
  try {
    ...
    semana.contenido_html = semana.contenido_html || "";
    semana.archivos_pdf   = semana.archivos_pdf   || [];
  }
  ...
  if (subtituloEditor && unidad) subtituloEditor.textContent = unidad.descripcion;
...
async function guardarEditor() {
  ...
  if (!num) { mostrarToast("No hay semana seleccionada", "error"); return; }
  ...
  if (btnGuardar) { btnGuardar.innerHTML = '<span class="spinner"></span> Guardando...'; btnGuardar.disabled = true; }
  ...
  try {
    ...
    const { data: existe } = await window.supabaseClient
      .from('semanas').select('numero_semana').eq('numero_semana', num).single();

    let error;
    if (existe) {
      const { error: e } = await window.supabaseClient.from('semanas').update(updateData).eq('numero_semana', num);
      error = e;
    } else {
      const { error: e } = await window.supabaseClient.from('semanas').insert({ numero_semana: num, ...updateData });
      error = e;
    }
    if (error) throw error;
    ...
    mostrarToast("❌ Error al guardar: " + (error.message || "Desconocido"), "error");
  } finally {
...
async function togglePublicar(numero) {
  ...
  try {
    const { error } = await window.supabaseClient
      .from('semanas')
      .update({ publicado: !semana.publicado, ultima_modificacion: new Date().toISOString() })
      .eq('numero_semana', numero);
    if (error) throw error;
    mostrarToast(`Semana ${numero} ${!semana.publicado ? "publicada" : "despublicada"} correctamente.`, "success");
    await cargarSemanas();
  } catch (error) {
...
async function limpiarSemana(numero) {
  if (!confirm(`¿Seguro que deseas limpiar el contenido de la Semana ${numero}? Esta acción no se puede deshacer.`)) return;
  try {
    const { error } = await window.supabaseClient
      .from('semanas')
      .update({ titulo: "", descripcion: "", contenido_html: "", archivos_pdf: [], publicado: false, ultima_modificacion: new Date().toISOString() })
      .eq('numero_semana', numero);
    if (error) throw error;
...
function inicializarEditorPdf(semana) {
  ...
  if (dropZone) {
    dropZone.ondragover  = function(e) { e.preventDefault(); dropZone.classList.add("pdf-dropzone-activa"); };
    dropZone.ondragleave = function()  { dropZone.classList.remove("pdf-dropzone-activa"); };
    dropZone.ondrop      = function(e) { e.preventDefault(); dropZone.classList.remove("pdf-dropzone-activa"); procesarArchivos(e.dataTransfer.files); };
  }
}
...
function procesarArchivos(archivos) {
  ...
  Array.from(archivos).forEach(function (archivo) {
    if (archivo.size > MAX_BYTES) { mostrarToast(`"${archivo.name}" supera el límite de 10 MB.`, "error"); return; }
    if (estadoAdmin.pdfsEditando.some(p => p.nombre === archivo.name)) { mostrarToast(`"${archivo.name}" ya está adjunto.`, "info"); return; }
    const info   = obtenerInfoTipo(archivo.name);
    const lector = new FileReader();
    lector.onload = function(ev) {
      estadoAdmin.pdfsEditando.push({ nombre: archivo.name, tamaño: archivo.size, base64: ev.target.result, categoria: info.categoria, visor: info.visor, etiqueta: info.etiqueta, icono: info.icono });
      renderizarListaPdfAdmin();
      mostrarToast(`"${archivo.name}" adjuntado ✓`, "success");
    };
    lector.onerror = function() { mostrarToast(`Error al leer "${archivo.name}".`, "error"); };
...
function renderizarListaPdfAdmin() {
  ...
  filtrados.forEach(function (archivo) {
    ...
    item.innerHTML = `
      ...
      <button type="button" onclick="eliminarPdfAdmin(${indiceReal})" class="btn btn-danger btn-sm">🗑️ Quitar</button>`;
    lista.appendChild(item);
  });

  const contador = document.createElement("p");
  contador.style.cssText = "font-size:0.75rem;color:var(--text-muted);margin-top:6px;text-align:right;";
  contador.textContent = `${estadoAdmin.pdfsEditando.length} archivo(s) adjunto(s)`;
...
async function cargarUnidades() {
  try {
    const { data, error } = await window.supabaseClient
      .from('unidades').select('*').order('numero', { ascending: true });
  ...
  UNIDADES.forEach(function(unidad) {
    ...
    card.innerHTML = `
      ...
      <div class="form-group" style="margin-bottom:0.75rem;">
        ...
        <input type="text" id="unidadNombre${unidad.numero}" class="form-input" value="${escapeHtml(unidad.nombre)}" maxlength="60" placeholder="Ej: Unidad I — Introducción">
      </div>
      <div class="form-group" style="margin-bottom:0;">
        <label class="form-label" for="unidadDesc${unidad.numero}">Descripción</label>
        <input type="text" id="unidadDesc${unidad.numero}" class="form-input" value="${escapeHtml(unidad.descripcion)}" maxlength="120" placeholder="Breve descripción de los temas cubiertos">
      </div>`;
...
async function guardarUnidades() {
  ...
  if (!valido) { mostrarAlertaUnidades("El nombre de cada unidad es obligatorio.", "error"); return; }

  nuevasUnidades.forEach(function(u, i) { UNIDADES[i].nombre = u.nombre; UNIDADES[i].descripcion = u.descripcion; });
  localStorage.setItem(UNIDADES_STORAGE_KEY, JSON.stringify(UNIDADES));
  ...
  try {
    for (const u of UNIDADES) {
      const { error } = await window.supabaseClient
        .from('unidades')
        .upsert({ numero: u.numero, nombre: u.nombre, descripcion: u.descripcion }, { onConflict: 'numero' });
      if (error) throw error;
    }
    guardadoEnBD = true;
...
function restablecerUnidades() {
  ...
  UNIDADES_DEFAULT.forEach(function(def, i) { UNIDADES[i].nombre = def.nombre; UNIDADES[i].descripcion = def.descripcion; });
...
function mostrarToast(mensaje, tipo) {
  ...
  toast.innerHTML = `<span style="font-weight:700;font-size:1rem;">${iconos[tipo] || "•"}</span><span>${mensaje}</span>`;
  contenedor.appendChild(toast);
  setTimeout(function() {
    toast.classList.add("removing");
    setTimeout(function() { if (toast.parentNode === contenedor) contenedor.removeChild(toast); }, 300);
  }, 3500);
}
...
function actualizarValor(id, valor) {
  const el = document.getElementById(id);
  if (!el) return;
  if (el.tagName === "INPUT" || el.tagName === "TEXTAREA") el.value = valor || "";
